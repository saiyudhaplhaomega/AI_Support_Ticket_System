...
| Script | What it does |
|---|---|
| `build_workflow.py` | **Generates** `workflow.noavia-ticket-pipeline.v1.json`. The tracked export is generated, not hand-edited - change this file, not the JSON. Deterministic and safe to re-run. |
| `verify_workflows.py` | Credential-free structural checks across every workflow export: required nodes exist, validation strings are present, routing thresholds intact, ingestion and retrieval embedding dimensions match. Prints `PASS:` on success. |
...
`build_workflow.py` reads the tracked export, strips the AI and routing nodes,
