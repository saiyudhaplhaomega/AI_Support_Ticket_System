...
return [{ json: { ...$json, _draft_request: { model: 'gpt-4o-mini', temperature: 0.2, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: `Write a professional customer-support DRAFT. Tone: ${tone}. ${instruction} Never claim that the draft was sent. Return JSON only: {"draft_response":"..."}.` }, { role: 'user', content: `Ticket subject: ${$json.ticket.subject}\\nTicket message: ${$json.ticket.body}\\nClassification: ${JSON.stringify($json.classification ?? {})}\\nKnowledge context:\\n${context || '(no matching policy)'}` }] } } }];"""},
